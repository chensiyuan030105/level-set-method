...
class GUIVisualizer2D(Visualizer2D):
    def __init__(self, grid_res, res, mode, output = True, text = False, title = 'demo'):
        super().__init__(grid_res, res)

        self.mode = mode
        self.gui = ti.GUI(title, (self.res, self.res), show_gui=False)
        self.output = output
        self.frame = 0
        self.text = text
    ...
    def fill_H(self, Hx : ti.template(), Hy : ti.template()):
        for i, j in self.color_buffer:
            ...
            if ti.abs(Hx[x, y]) < 1e-4 and ti.abs(Hy[x, y]) < 1e-4:
                ...
                H = ti.normalize(ti.Vector([Hx[x, y], Hy[x, y]]))
    ...
    def visualize(self, simulator):
        ...
        if self.output:
            self.gui.show(f'results/single_vortex/image_redistance/{self.frame:06d}.png')
        else:
            self.gui.show()
